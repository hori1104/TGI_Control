...
def get_target_temp_X(elapsed_time):
    ...
    if elapsed_time <= 30.0:
        ...
        cycle_time = (elapsed_time - 30) % (4 * target_time)  
        if cycle_time <= target_time:  
            return -(5 / target_time) * cycle_time + 25.0
        else: 
            return (5 / (3 * target_time)) * (cycle_time - target_time) + 20.0
def get_target_temp_Y(elapsed_time):
    global target_time
    if elapsed_time <= (30 + target_time):  
        return 25.0
    else:
        cycle_time = (elapsed_time - (30 + target_time)) % (4 * target_time)  
        if cycle_time <= target_time:  
            return -(5 / target_time) * cycle_time + 25.0
        else:  
            return (5 / (3 * target_time)) * (cycle_time - target_time) + 20.0
def get_target_temp_L(elapsed_time):
    global target_time
    if elapsed_time <= (30 + (2 * target_time)): 
        return 25.0
    else:
        cycle_time = (elapsed_time - (30 + (2 * target_time))) % (4 * target_time) 
        if cycle_time <= target_time:  
            return -(5 / target_time) * cycle_time + 25.0
        else: 
            return (5 / (3 * target_time)) * (cycle_time - target_time) + 20.0
def get_target_temp_Z(elapsed_time):
    global target_time
    if elapsed_time <= (30 + (3 * target_time)): 
        return 25.0
    else:
        cycle_time = (elapsed_time - (30 + (3 * target_time))) % (4 * target_time) 
        if cycle_time <= target_time: 
            return -(5 / target_time) * cycle_time + 25.0
        else:  
            return (5 / (3 * target_time)) * (cycle_time - target_time) + 20.0
...
while True:
    ...
    if  temp_X <= 25.5 and temp_Y <= 25.5 and temp_Z <= 25.5 and temp_L <= 25.5 and temp_N >= 30.0 and temp_M >= 30.0: 
        break
...
with open('opamp.csv', 'w', newline='') as csvfile:
    ...
    while True:
        ...
        target_temp_Z = get_target_temp_Z(elapsed_time)
        target_temp_L = get_target_temp_L(elapsed_time)
